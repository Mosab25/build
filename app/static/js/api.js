...
const AdminAPI = {
  ...
  deletePayment: (id, reason = "") => apiDelete(`/api/admin/payments/${id}?confirm=true${reason ? `&reason=${encodeURIComponent(reason)}` : ""}`),
...
const OwnerAPI = {
  ...
  deals: async (page = 1, limit = 20) => listPayload(await apiGet(`/api/owner/deals?page=${page}&limit=${limit}`), "deals"),
  ...
  clients: async (page = 1, limit = 20) => listPayload(await apiGet(`/api/owner/clients?page=${page}&limit=${limit}`), "clients"),
  ...
  payments: async (page = 1, limit = 20) => listPayload(await apiGet(`/api/owner/payments?page=${page}&limit=${limit}`), "payments"),
  ...
  auditLogs: async (page = 1, limit = 20) => listPayload(await apiGet(`/api/owner/audit-logs?page=${page}&limit=${limit}`), "auditLogs"),
