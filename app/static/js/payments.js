...
async function openPaymentForm(clientId = "", payment = null) {
  ...
  function populateApartmentsForClient(clientId) {
    ...
    list.forEach((a) => {
      const apartment = a?.apartment || a || {};
      const value = a.id || a.apartmentId || apartment.id || a.client_apartment_id || "";
      const label = `${escapeHTML(a.unitCode || a.unit_code || apartment.unitCode || apartment.unit_code || "-" )} - ${formatMoney(a.price ?? a.totalAmount ?? a.unitPrice ?? a.unit_price ?? apartment.price ?? 0)}`;
