...
function showStaffApp() {
  ...
  if (isMobile && !sessionStorage.getItem("dashboardSidebarOpenedOnce")) {
    openDashboardSidebarDrawer();
    sessionStorage.setItem("dashboardSidebarOpenedOnce", "1");
  }
}
...
function renderDashboardShell() {
  ...
  const roleDesktop = qs("#dashboardRoleLabelDesktop");
  const titleDesktop = qs("#dashboardTitleDesktop");
  const roleSidebar = qs("#dashboardRoleLabelSidebar");
  const titleSidebar = qs("#dashboardTitleSidebar");
  if (roleDesktop) roleDesktop.textContent = qs("#dashboardRoleLabel")?.textContent || "";
  if (titleDesktop) titleDesktop.textContent = qs("#dashboardTitle")?.textContent || "";
  if (roleSidebar) roleSidebar.textContent = qs("#dashboardRoleLabel")?.textContent || "";
  if (titleSidebar) titleSidebar.textContent = qs("#dashboardTitle")?.textContent || "";
