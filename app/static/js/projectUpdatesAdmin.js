    ...
    if (isMobile) {
        ...
        <div class="updates-admin-mobile-list">
          ${updates.map((item) => `
            <article class="updates-admin-mobile-card">
              ...
              <div class="updates-admin-mobile-actions horizontal-actions">
      ...
      target.innerHTML = `
        <div class="table-wrap updates-admin-table-wrap">
          <table>
            <thead><tr><th>الوسائط</th><th>العنوان</th><th>التاريخ</th><th>الحالة</th><th>إجراءات</th></tr></thead>
            <tbody>${updates.map((item) => `
              <tr>
                <td data-label="الوسائط">${renderAdminUpdateMedia(item)}</td>
                ...
                <td data-label="إجراءات" class="table-actions horizontal-actions">
                  ${updatesAdminFilter === "active" ? `
                    ${item.status === "published"
                      ? `<button class="btn ghost small" data-update-unpublish="${item.id}">إلغاء النشر</button>`
                      : `<button class="btn primary small" data-update-publish="${item.id}">نشر</button>`}
