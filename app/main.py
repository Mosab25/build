...
from app.config import GENERATED_DIR, MEDIA_DIR, PROJECT_ROOT, STATIC_DIR, TEMPLATES_DIR
from app.utils.db_utils import db, get_database_url, get_db_connection as checkout_db_connection, release_db_connection, recalc_client
from scripts.bootstrap_owner import bootstrap_owner_account
...
def project_update_mime_type(url: str | None = None, extension: str | None = None, fallback: str | None = None) -> str:
    clean_extension = (extension or "").lower().strip().lstrip(".")
    if not clean_extension and url:
        clean_url = url.split("?", 1)[0].split("#", 1)[0]
        clean_extension = Path(clean_url).suffix.lower().lstrip(".")
    if clean_extension in UPDATE_MIME_TYPES:
        return UPDATE_MIME_TYPES[clean_extension]
    guessed, _ = mimetypes.guess_type(url or "")
    return guessed or fallback or "application/octet-stream"


def project_update_media_type(extension: str) -> str:
    return "video" if extension.lower() in UPDATE_VIDEO_EXTENSIONS else "image"


def cloudinary_upload_configured() -> bool:
    return all(
        os.environ.get(name)
...
def upload_project_update_to_cloudinary(uploaded, extension: str) -> dict[str, str]:
    try:
        import cloudinary
        import cloudinary.uploader
    except ImportError as exc:
        raise RuntimeError("Cloudinary is configured but the cloudinary package is not installed.") from exc
    ...
    media_type = project_update_media_type(extension)
    result = cloudinary.uploader.upload(
        uploaded.stream,
        folder="real-estate/project-updates",
        resource_type="video" if media_type == "video" else "image",
        public_id=public_id("project_update_media"),
        overwrite=False,
    )
    ...
    return {
        ...
        "mimeType": project_update_mime_type(secure_url, extension),
        "provider": "cloudinary",
    }
...
def portfolio_key_for_client(client: dict[str, Any]) -> str:
    return normalize_code(client.get("portfolio_code") or client.get("client_code") or client.get("id") or "")


def rows_for_client_portfolio(conn: Any, client: dict[str, Any]) -> list[dict[str, Any]]:
    key = portfolio_key_for_client(client)
    if not key:
        return [client]
    rows = rows_for_portfolio_code(conn, key)
    return rows or [client]


def recalc_portfolio_rows(conn: Any, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not rows:
        return []
    key = portfolio_key_for_client(rows[0])
    for row in rows:
        recalc_client(conn, row["id"])
    if key:
        refreshed = rows_for_portfolio_code(conn, key)
        if refreshed:
            return refreshed
    ids = [row["id"] for row in rows]
    return [conn.execute("SELECT * FROM clients WHERE id = ?", (client_id,)).fetchone() for client_id in ids]
...
def portfolio_payload(conn: Any, rows: list[dict[str, Any]], include_private: bool = False) -> dict[str, Any]:
    ...
    paid_amount = sum(
        ...
        if str(payment.get("status") or "").lower().replace(" ", "_") == "confirmed"
    )
...
def owner_dashboard_summary_payload(conn: Any) -> dict[str, Any]:
    active_rows = conn.execute("SELECT id, portfolio_code, client_code FROM clients WHERE reservation_status != 'cancelled'").fetchall()
    total_clients = len({portfolio_key_for_client(row) or row["id"] for row in active_rows})
...
def list_admin_clients() -> Response:
    ...
    with db() as conn:
        ...
        for group_rows in groups.values():
            ...
            grouped_items.append(portfolio_payload(conn, refreshed_rows, include_private=False))
...
def client_financial_summary(client_id: str) -> Response:
    ...
    with db() as conn:
        ...
        rows = recalc_portfolio_rows(conn, rows_for_client_portfolio(conn, client))
        payload = portfolio_payload(conn, rows, include_private=True)
        return jsonify({
            "financial_summary": {
                "client_id": client_id,
                ...
                "apartments": [{"id": apt.get("id"), "unit_code": apt.get("unitCode"), "price": apt.get("price")} for apt in payload.get("apartments", [])],
            }
        })
...
def add_payment() -> Response:
    ...
    with db() as conn:
        ...
        if not client:
            return jsonify({"error": "not_found", "message": "العميل غير موجود."}), 404
        portfolio_rows = recalc_portfolio_rows(conn, rows_for_client_portfolio(conn, client))
        portfolio = portfolio_payload(conn, portfolio_rows, include_private=True)
        portfolio_apartment_ids = {apt.get("id") for apt in portfolio.get("apartments", []) if apt.get("id")}
        ...
        if apartment_id:
            ...
            if apartment_id not in portfolio_apartment_ids:
                return jsonify({"error": "assignment", "message": "هذه الشقة مرتبطة بعميل آخر."}), 409
        if amount > float(portfolio["remainingAmount"] or portfolio["totalAmount"]) and not payload.get("allow_overpay"):
        ...
        recalc_portfolio_rows(conn, portfolio_rows)
...
def get_all_project_updates() -> Response:
    ...
    with db() as conn:
        ...
        return jsonify(paginated_payload("updates", [project_update_payload(update) for update in updates], total, page, limit))
...
def create_project_update() -> Response:
    ...
    if media_url:
        ...
        if payload.get("media_type") == "video" and not inferred_mime_type.startswith("video/"):
            return jsonify({"error": "validation", "message": "نوع الوسائط لا يطابق ملف الفيديو المرفوع."}), 400
        if payload.get("media_type") == "image" and not inferred_mime_type.startswith("image/"):
...
def update_project_update(update_id: str) -> Response:
    ...
    with db() as conn:
        ...
        if update_fields:
            update_values.extend([now_iso(), update_id])
            conn.execute(
...
def upload_project_update_media() -> Response:
    ...
    if extension not in ALLOWED_UPDATE_EXTENSIONS:
        return jsonify({"error": "validation", "message": "نوع الملف غير مسموح. الصيغ المسموحة: jpg, jpeg, png, webp, mp4, webm. لا يتم دعم mov حاليًا."}), 400

    expected_mime_type = project_update_mime_type(extension=extension)
    uploaded_mime_type = (uploaded.mimetype or "").split(";", 1)[0].strip().lower()
    tolerated_unknown_types = {"", "application/octet-stream", "binary/octet-stream"}
    if uploaded_mime_type not in tolerated_unknown_types and uploaded_mime_type != expected_mime_type:
        if extension in UPDATE_VIDEO_EXTENSIONS:
            return jsonify({"error": "validation", "message": "نوع الفيديو غير مدعوم. استخدم mp4 أو webm فقط."}), 400
        return jsonify({"error": "validation", "message": "نوع الصورة غير مدعوم. استخدم jpg أو png أو webp فقط."}), 400
    ...
    media_type = project_update_media_type(extension)
    if cloudinary_upload_configured():
        try:
            upload_result = upload_project_update_to_cloudinary(uploaded, extension)
        except Exception as exc:
            return jsonify({"error": "upload_failed", "message": f"تعذر رفع الملف إلى التخزين السحابي: {str(exc)}"}), 502
