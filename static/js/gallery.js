...
function openMedia(key) {
  ...
  openModal(`
    ...
    ${item.type === "video"
      ? `<video controls autoplay muted playsinline poster="${item.poster}" style="width:100%;border-radius:8px;background:#000"><source src="${item.src}" type="video/mp4" /></video>`
      : `<img src="${item.src}" alt="${escapeHTML(item.alt || item.title)}" style="width:100%;border-radius:8px" />`}
  `);
}
